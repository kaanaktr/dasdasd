...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}#${req.user.discriminator}\` Adlı Kullanıcı \`${sistem.id}\` ID'sine Sahip \`${sistem.username}#${sistem.discriminator}\` Adlı Botu İle Başvuru Yaptı!`)

if (client.users.has(req.user.id) === true) {
  client.users.get(req.user.id).send(`\`${sistem.username}#${sistem.discriminator}\` Adlı Botunuz Başarıyla Sistemimize Eklendi! Şimdi, Tek Yapmanız Gereken Şey Oturup Sıcak Bir Çayın Yanında Botun Onaylanmasını Beklemektir.\n İyi Şanslar! 🎉`)
}
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}#${req.user.discriminator}\` Adlı Kullanıcı \`${sistem.id}\` ID'sine Sahip \`${sistem.username}#${sistem.discriminator}\` Adlı Botunun Başvurusunu/Profilini Düzenlendi!`)
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}#${req.user.discriminator}\` adlı yetkili tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` adlı kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine sahip \`${db.fetch(`botlar.${id}.isim`)}\` adlı botu onaylandı!`)
...
client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}#${req.user.discriminator}\` adlı yetkili tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` adlı kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine sahip \`${db.fetch(`botlar.${id}.isim`)}\` adlı botu bekleme moduna alındı!`)
...
app.post("/botyonetici/reddet/:botID", checkAuth, (req, res) => {
  ...
  client.channels.get(client.ayarlar.kayıt).send(`\`${req.user.username}#${req.user.discriminator}\` adlı yetkili tarafından \`${db.fetch(`botlar.${id}.sahip`)}\` adlı kullanıcının \`${db.fetch(`botlar.${id}.id`)}\` ID'sine sahip \`${db.fetch(`botlar.${id}.isim`)}\` adlı botu \`${req.body['red-sebep']}\` sebebi ile reddedildi!`)
